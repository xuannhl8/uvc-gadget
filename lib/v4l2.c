...
int v4l2_dequeue_buffer(struct v4l2_device *dev, struct video_buffer *buffer)
{
	...
	// printf("Call back dequeue buffer\n");
	...
	if (last.tv_sec != 0 || last.tv_usec != 0) {
        elapsed = (end.tv_sec - last.tv_sec) + 
                  (end.tv_usec - last.tv_usec) / 1000000.0;
		fps_btw = 1.0 / elapsed;
        // printf("Elapsed time between two buffer: %f s. FPS = %.2f\n", elapsed, fps_btw);
    }
	// Save last value
	last = end;
	...
	buffer->bytesused = buf.bytesused; /* Reason for byteused  = Height * Weight * Bit Color */
